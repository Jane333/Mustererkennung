...
% Unterraum erzeugen
pca_ur_10dim  = EigVec_CVM_A(:,biggestEVec -9:biggestEVec);

% Abbildung der Trainingsdaten auf Unterraum
A_0_ur_dim10  = A_0_nl * pca_ur_10dim; % Datenpunkte fuer Zuglinie 0
A_1_ur_dim10  = A_1_nl * pca_ur_10dim; % Datenpunkte fuer Zuglinie 1
A_2_ur_dim10  = A_2_nl * pca_ur_10dim; % Datenpunkte fuer Zuglinie 2
A_3_ur_dim10  = A_3_nl * pca_ur_10dim; % Datenpunkte fuer Zuglinie 3
A_4_ur_dim10  = A_4_nl * pca_ur_10dim; % Datenpunkte fuer Zuglinie 4
A_5_ur_dim10  = A_5_nl * pca_ur_10dim; % Datenpunkte fuer Zuglinie 5
A_6_ur_dim10  = A_6_nl * pca_ur_10dim; % Datenpunkte fuer Zuglinie 6
A_7_ur_dim10  = A_7_nl * pca_ur_10dim; % Datenpunkte fuer Zuglinie 7
A_8_ur_dim10  = A_8_nl * pca_ur_10dim; % Datenpunkte fuer Zuglinie 8
A_9_ur_dim10  = A_9_nl * pca_ur_10dim; % Datenpunkte fuer Zuglinie 9

% Abbidung der Testdaten auf Unterraum
B_ur_dim10  = B_nl * pca_ur_10dim;
...
% Klassifizierung der Testdaten (Metrik: L2-Norm)
M_classify_dim10 = [];
for index = 1:size(B_ur_dim10,1)
    
    trainData = B_ur_dim10(index,:);
    
    % multivariate PDF fuer Testdatensatz (für jede Zuglinie)
    A_0_aposteriori_predict_dim10 = mvnpdf(trainData, E_A_0_ur_dim10, CVM_A_0_ur_dim10) * A_x_apriori;
    A_1_aposteriori_predict_dim10 = mvnpdf(trainData, E_A_1_ur_dim10, CVM_A_1_ur_dim10) * A_x_apriori;
    A_2_aposteriori_predict_dim10 = mvnpdf(trainData, E_A_2_ur_dim10, CVM_A_2_ur_dim10) * A_x_apriori;
    A_3_aposteriori_predict_dim10 = mvnpdf(trainData, E_A_3_ur_dim10, CVM_A_3_ur_dim10) * A_x_apriori;
    A_4_aposteriori_predict_dim10 = mvnpdf(trainData, E_A_4_ur_dim10, CVM_A_4_ur_dim10) * A_x_apriori;
    A_5_aposteriori_predict_dim10 = mvnpdf(trainData, E_A_5_ur_dim10, CVM_A_5_ur_dim10) * A_x_apriori;
    A_6_aposteriori_predict_dim10 = mvnpdf(trainData, E_A_6_ur_dim10, CVM_A_6_ur_dim10) * A_x_apriori;
    A_7_aposteriori_predict_dim10 = mvnpdf(trainData, E_A_7_ur_dim10, CVM_A_7_ur_dim10) * A_x_apriori;
    A_8_aposteriori_predict_dim10 = mvnpdf(trainData, E_A_8_ur_dim10, CVM_A_8_ur_dim10) * A_x_apriori;
    A_9_aposteriori_predict_dim10 = mvnpdf(trainData, E_A_9_ur_dim10, CVM_A_9_ur_dim10) * A_x_apriori;
    
    % L2 Norm der aposteriori Vorhersage
    A0_l2_dim10 = norm(A_0_aposteriori_predict_dim10);
    A1_l2_dim10 = norm(A_1_aposteriori_predict_dim10);
    A2_l2_dim10 = norm(A_2_aposteriori_predict_dim10);
    A3_l2_dim10 = norm(A_3_aposteriori_predict_dim10);
    A4_l2_dim10 = norm(A_4_aposteriori_predict_dim10);
    A5_l2_dim10 = norm(A_5_aposteriori_predict_dim10);
    A6_l2_dim10 = norm(A_6_aposteriori_predict_dim10);
    A7_l2_dim10 = norm(A_7_aposteriori_predict_dim10);
    A8_l2_dim10 = norm(A_8_aposteriori_predict_dim10);
    A9_l2_dim10 = norm(A_9_aposteriori_predict_dim10);
    
    % Bestimmung des Maximums (aposteriori Vorhersage)
    [maxValue, indexAtMaxValue] = max([A0_l2_dim10, A1_l2_dim10, A2_l2_dim10, A3_l2_dim10, A4_l2_dim10, A5_l2_dim10, A6_l2_dim10, A7_l2_dim10, A8_l2_dim10, A9_l2_dim10]);
    
    % Bayes Klassifikation (Welche aposteriori Vorhersage war die Groeßte?)
    if (maxValue == A0_l2_dim10)       % train 0 predicted
        tmpVector = [B_ur_dim10(index,:),B(index,B_n),0];
        M_classify_dim10 = vertcat(M_classify_dim10,tmpVector);
    elseif (maxValue == A1_l2_dim10)   % train 1 predicted
        tmpVector = [B_ur_dim10(index,:),B(index,B_n),1];
        M_classify_dim10 = vertcat(M_classify_dim10,tmpVector);
    elseif (maxValue == A2_l2_dim10)   % train 2 predicted
        tmpVector = [B_ur_dim10(index,:),B(index,B_n),2];
        M_classify_dim10 = vertcat(M_classify_dim10,tmpVector);
    elseif (maxValue == A3_l2_dim10)   % train 3 predicted
        tmpVector = [B_ur_dim10(index,:),B(index,B_n),3];
        M_classify_dim10 = vertcat(M_classify_dim10,tmpVector);
    elseif (maxValue == A4_l2_dim10)   % train 4 predicted
        tmpVector = [B_ur_dim10(index,:),B(index,B_n),4];
        M_classify_dim10 = vertcat(M_classify_dim10,tmpVector);
    elseif (maxValue == A5_l2_dim10)   % train 5 predicted
        tmpVector = [B_ur_dim10(index,:),B(index,B_n),5];
        M_classify_dim10 = vertcat(M_classify_dim10,tmpVector);
    elseif (maxValue == A6_l2_dim10)   % train 6 predicted
        tmpVector = [B_ur_dim10(index,:),B(index,B_n),6];
        M_classify_dim10 = vertcat(M_classify_dim10,tmpVector);
    elseif (maxValue == A7_l2_dim10)   % train 7 predicted
        tmpVector = [B_ur_dim10(index,:),B(index,B_n),7];
        M_classify_dim10 = vertcat(M_classify_dim10,tmpVector);
    elseif (maxValue == A8_l2_dim10)   % train 8 predicted
        tmpVector = [B_ur_dim10(index,:),B(index,B_n),8];
        M_classify_dim10 = vertcat(M_classify_dim10,tmpVector);
    else                              % train 9 predicted
        tmpVector = [B_ur_dim10(index,:),B(index,B_n),9];
        M_classify_dim10 = vertcat(M_classify_dim10,tmpVector);
    end % end-if
end % end-for_each

M_classify_dim10_n = size(M_classify_dim10,2);
M_classify_dim10_m = size(M_classify_dim10,1);

% Konfusionsmatrix
knownClass_dim10 = M_classify_dim10(:, M_classify_dim10_n -1);
predictedClass_dim10 = M_classify_dim10(:, M_classify_dim10_n);
confusionmatrix_dim10 = confusionmat(knownClass, predictedClass)

%  confusionmatrix_dim10 =
%
%  341     0     0     0     0     0     0     0    22     0
%    0   350    12     0     1     0     0     0     1     0
%    0     8   355     0     0     0     0     1     0     0
...
% Klassifikationsguete
corret_predicted_dim10 = 0;
for index = 1:M_classify_dim10_m
    if M_classify_dim10(index, M_classify_dim10_n -1) == M_classify_dim10(index, M_classify_dim10_n)
        corret_predicted_dim10 = corret_predicted_dim10 + 1;
    end
end
classification_quality_dim10 = corret_predicted_dim10 / M_classify_dim10_m

%  classification_quality_dim10 = 0.9480

%%%%% PCA: 11-dimmensional %%%%%

% Unterraum erzeugen
pca_ur_11dim  = EigVec_CVM_A(:,biggestEVec -10:biggestEVec);

% Abbildung der Trainingsdaten auf Unterraum
A_0_ur_dim11  = A_0_nl * pca_ur_11dim; % Datenpunkte fuer Zuglinie 0
A_1_ur_dim11  = A_1_nl * pca_ur_11dim; % Datenpunkte fuer Zuglinie 1
A_2_ur_dim11  = A_2_nl * pca_ur_11dim; % Datenpunkte fuer Zuglinie 2
A_3_ur_dim11  = A_3_nl * pca_ur_11dim; % Datenpunkte fuer Zuglinie 3
A_4_ur_dim11  = A_4_nl * pca_ur_11dim; % Datenpunkte fuer Zuglinie 4
A_5_ur_dim11  = A_5_nl * pca_ur_11dim; % Datenpunkte fuer Zuglinie 5
A_6_ur_dim11  = A_6_nl * pca_ur_11dim; % Datenpunkte fuer Zuglinie 6
A_7_ur_dim11  = A_7_nl * pca_ur_11dim; % Datenpunkte fuer Zuglinie 7
A_8_ur_dim11  = A_8_nl * pca_ur_11dim; % Datenpunkte fuer Zuglinie 8
A_9_ur_dim11  = A_9_nl * pca_ur_11dim; % Datenpunkte fuer Zuglinie 9

% Abbidung der Testdaten auf Unterraum
B_ur_dim11  = B_nl * pca_ur_11dim;
...
% Klassifizierung der Testdaten (Metrik: L2-Norm)
M_classify_dim11 = [];
for index = 1:size(B_ur_dim11,1)
    
    trainData = B_ur_dim11(index,:);
    
    % multivariate PDF fuer Testdatensatz (für jede Zuglinie)
    A_0_aposteriori_predict_dim11 = mvnpdf(trainData, E_A_0_ur_dim11, CVM_A_0_ur_dim11) * A_x_apriori;
    A_1_aposteriori_predict_dim11 = mvnpdf(trainData, E_A_1_ur_dim11, CVM_A_1_ur_dim11) * A_x_apriori;
    A_2_aposteriori_predict_dim11 = mvnpdf(trainData, E_A_2_ur_dim11, CVM_A_2_ur_dim11) * A_x_apriori;
    A_3_aposteriori_predict_dim11 = mvnpdf(trainData, E_A_3_ur_dim11, CVM_A_3_ur_dim11) * A_x_apriori;
    A_4_aposteriori_predict_dim11 = mvnpdf(trainData, E_A_4_ur_dim11, CVM_A_4_ur_dim11) * A_x_apriori;
    A_5_aposteriori_predict_dim11 = mvnpdf(trainData, E_A_5_ur_dim11, CVM_A_5_ur_dim11) * A_x_apriori;
    A_6_aposteriori_predict_dim11 = mvnpdf(trainData, E_A_6_ur_dim11, CVM_A_6_ur_dim11) * A_x_apriori;
    A_7_aposteriori_predict_dim11 = mvnpdf(trainData, E_A_7_ur_dim11, CVM_A_7_ur_dim11) * A_x_apriori;
    A_8_aposteriori_predict_dim11 = mvnpdf(trainData, E_A_8_ur_dim11, CVM_A_8_ur_dim11) * A_x_apriori;
    A_9_aposteriori_predict_dim11 = mvnpdf(trainData, E_A_9_ur_dim11, CVM_A_9_ur_dim11) * A_x_apriori;
    
    % L2 Norm der aposteriori Vorhersage
    A0_l2_dim11 = norm(A_0_aposteriori_predict_dim11);
    A1_l2_dim11 = norm(A_1_aposteriori_predict_dim11);
    A2_l2_dim11 = norm(A_2_aposteriori_predict_dim11);
    A3_l2_dim11 = norm(A_3_aposteriori_predict_dim11);
    A4_l2_dim11 = norm(A_4_aposteriori_predict_dim11);
    A5_l2_dim11 = norm(A_5_aposteriori_predict_dim11);
    A6_l2_dim11 = norm(A_6_aposteriori_predict_dim11);
    A7_l2_dim11 = norm(A_7_aposteriori_predict_dim11);
    A8_l2_dim11 = norm(A_8_aposteriori_predict_dim11);
    A9_l2_dim11 = norm(A_9_aposteriori_predict_dim11);
    
    % Bestimmung des Maximums (aposteriori Vorhersage)
    [maxValue, indexAtMaxValue] = max([A0_l2_dim11, A1_l2_dim11, A2_l2_dim11, A3_l2_dim11, A4_l2_dim11, A5_l2_dim11, A6_l2_dim11, A7_l2_dim11, A8_l2_dim11, A9_l2_dim11]);
    
    % Bayes Klassifikation (Welche aposteriori Vorhersage war die Groeßte?)
    if (maxValue == A0_l2_dim11)       % train 0 predicted
        tmpVector = [B_ur_dim11(index,:),B(index,B_n),0];
        M_classify_dim11 = vertcat(M_classify_dim11,tmpVector);
    elseif (maxValue == A1_l2_dim11)   % train 1 predicted
        tmpVector = [B_ur_dim11(index,:),B(index,B_n),1];
        M_classify_dim11 = vertcat(M_classify_dim11,tmpVector);
    elseif (maxValue == A2_l2_dim11)   % train 2 predicted
        tmpVector = [B_ur_dim11(index,:),B(index,B_n),2];
        M_classify_dim11 = vertcat(M_classify_dim11,tmpVector);
    elseif (maxValue == A3_l2_dim11)   % train 3 predicted
        tmpVector = [B_ur_dim11(index,:),B(index,B_n),3];
        M_classify_dim11 = vertcat(M_classify_dim11,tmpVector);
    elseif (maxValue == A4_l2_dim11)   % train 4 predicted
        tmpVector = [B_ur_dim11(index,:),B(index,B_n),4];
        M_classify_dim11 = vertcat(M_classify_dim11,tmpVector);
    elseif (maxValue == A5_l2_dim11)   % train 5 predicted
        tmpVector = [B_ur_dim11(index,:),B(index,B_n),5];
        M_classify_dim11 = vertcat(M_classify_dim11,tmpVector);
    elseif (maxValue == A6_l2_dim11)   % train 6 predicted
        tmpVector = [B_ur_dim11(index,:),B(index,B_n),6];
        M_classify_dim11 = vertcat(M_classify_dim11,tmpVector);
    elseif (maxValue == A7_l2_dim11)   % train 7 predicted
        tmpVector = [B_ur_dim11(index,:),B(index,B_n),7];
        M_classify_dim11 = vertcat(M_classify_dim11,tmpVector);
    elseif (maxValue == A8_l2_dim11)   % train 8 predicted
        tmpVector = [B_ur_dim11(index,:),B(index,B_n),8];
        M_classify_dim11 = vertcat(M_classify_dim11,tmpVector);
    else                              % train 9 predicted
        tmpVector = [B_ur_dim11(index,:),B(index,B_n),9];
        M_classify_dim11 = vertcat(M_classify_dim11,tmpVector);
    end % end-if
end % end-for_each

M_classify_dim11_n = size(M_classify_dim11,2);
M_classify_dim11_m = size(M_classify_dim11,1);

% Konfusionsmatrix
knownClass_dim11 = M_classify_dim11(:, M_classify_dim11_n -1);
predictedClass_dim11 = M_classify_dim11(:, M_classify_dim11_n);
confusionmatrix_dim11 = confusionmat(knownClass, predictedClass)

%  confusionmatrix_dim11 =
%
%  341     0     0     0     0     0     0     0    22     0
%    0   350    12     0     1     0     0     0     1     0
%    0     8   355     0     0     0     0     1     0     0
...
% Klassifikationsguete
corret_predicted_dim11 = 0;
for index = 1:M_classify_dim11_m
    if M_classify_dim11(index, M_classify_dim11_n -1) == M_classify_dim11(index, M_classify_dim11_n)
        corret_predicted_dim11 = corret_predicted_dim11 + 1;
    end
end
classification_quality_dim11 = corret_predicted_dim11 / M_classify_dim11_m

%  classification_quality_dim11 = 0.9537

%%%%% PCA: 12-dimmensional %%%%%

% Unterraum erzeugen
pca_ur_12dim  = EigVec_CVM_A(:,biggestEVec -11:biggestEVec);

% Abbildung der Trainingsdaten auf Unterraum
A_0_ur_dim12  = A_0_nl * pca_ur_12dim; % Datenpunkte fuer Zuglinie 0
A_1_ur_dim12  = A_1_nl * pca_ur_12dim; % Datenpunkte fuer Zuglinie 1
A_2_ur_dim12  = A_2_nl * pca_ur_12dim; % Datenpunkte fuer Zuglinie 2
A_3_ur_dim12  = A_3_nl * pca_ur_12dim; % Datenpunkte fuer Zuglinie 3
A_4_ur_dim12  = A_4_nl * pca_ur_12dim; % Datenpunkte fuer Zuglinie 4
A_5_ur_dim12  = A_5_nl * pca_ur_12dim; % Datenpunkte fuer Zuglinie 5
A_6_ur_dim12  = A_6_nl * pca_ur_12dim; % Datenpunkte fuer Zuglinie 6
A_7_ur_dim12  = A_7_nl * pca_ur_12dim; % Datenpunkte fuer Zuglinie 7
A_8_ur_dim12  = A_8_nl * pca_ur_12dim; % Datenpunkte fuer Zuglinie 8
A_9_ur_dim12  = A_9_nl * pca_ur_12dim; % Datenpunkte fuer Zuglinie 9

% Abbidung der Testdaten auf Unterraum
B_ur_dim12  = B_nl * pca_ur_12dim;
...
% Klassifizierung der Testdaten (Metrik: L2-Norm)
M_classify_dim12 = [];
for index = 1:size(B_ur_dim12,1)
    
    trainData = B_ur_dim12(index,:);
    
    % multivariate PDF fuer Testdatensatz (für jede Zuglinie)
    A_0_aposteriori_predict_dim12 = mvnpdf(trainData, E_A_0_ur_dim12, CVM_A_0_ur_dim12) * A_x_apriori;
    A_1_aposteriori_predict_dim12 = mvnpdf(trainData, E_A_1_ur_dim12, CVM_A_1_ur_dim12) * A_x_apriori;
    A_2_aposteriori_predict_dim12 = mvnpdf(trainData, E_A_2_ur_dim12, CVM_A_2_ur_dim12) * A_x_apriori;
    A_3_aposteriori_predict_dim12 = mvnpdf(trainData, E_A_3_ur_dim12, CVM_A_3_ur_dim12) * A_x_apriori;
    A_4_aposteriori_predict_dim12 = mvnpdf(trainData, E_A_4_ur_dim12, CVM_A_4_ur_dim12) * A_x_apriori;
    A_5_aposteriori_predict_dim12 = mvnpdf(trainData, E_A_5_ur_dim12, CVM_A_5_ur_dim12) * A_x_apriori;
    A_6_aposteriori_predict_dim12 = mvnpdf(trainData, E_A_6_ur_dim12, CVM_A_6_ur_dim12) * A_x_apriori;
    A_7_aposteriori_predict_dim12 = mvnpdf(trainData, E_A_7_ur_dim12, CVM_A_7_ur_dim12) * A_x_apriori;
    A_8_aposteriori_predict_dim12 = mvnpdf(trainData, E_A_8_ur_dim12, CVM_A_8_ur_dim12) * A_x_apriori;
    A_9_aposteriori_predict_dim12 = mvnpdf(trainData, E_A_9_ur_dim12, CVM_A_9_ur_dim12) * A_x_apriori;
    
    % L2 Norm der aposteriori Vorhersage
    A0_l2_dim12 = norm(A_0_aposteriori_predict_dim12);
    A1_l2_dim12 = norm(A_1_aposteriori_predict_dim12);
    A2_l2_dim12 = norm(A_2_aposteriori_predict_dim12);
    A3_l2_dim12 = norm(A_3_aposteriori_predict_dim12);
    A4_l2_dim12 = norm(A_4_aposteriori_predict_dim12);
    A5_l2_dim12 = norm(A_5_aposteriori_predict_dim12);
    A6_l2_dim12 = norm(A_6_aposteriori_predict_dim12);
    A7_l2_dim12 = norm(A_7_aposteriori_predict_dim12);
    A8_l2_dim12 = norm(A_8_aposteriori_predict_dim12);
    A9_l2_dim12 = norm(A_9_aposteriori_predict_dim12);
    
    % Bestimmung des Maximums (aposteriori Vorhersage)
    [maxValue, indexAtMaxValue] = max([A0_l2_dim12, A1_l2_dim12, A2_l2_dim12, A3_l2_dim12, A4_l2_dim12, A5_l2_dim12, A6_l2_dim12, A7_l2_dim12, A8_l2_dim12, A9_l2_dim12]);
    
    % Bayes Klassifikation (Welche aposteriori Vorhersage war die Groeßte?)
    if (maxValue == A0_l2_dim12)       % train 0 predicted
        tmpVector = [B_ur_dim12(index,:),B(index,B_n),0];
        M_classify_dim12 = vertcat(M_classify_dim12,tmpVector);
    elseif (maxValue == A1_l2_dim12)   % train 1 predicted
        tmpVector = [B_ur_dim12(index,:),B(index,B_n),1];
        M_classify_dim12 = vertcat(M_classify_dim12,tmpVector);
    elseif (maxValue == A2_l2_dim12)   % train 2 predicted
        tmpVector = [B_ur_dim12(index,:),B(index,B_n),2];
        M_classify_dim12 = vertcat(M_classify_dim12,tmpVector);
    elseif (maxValue == A3_l2_dim12)   % train 3 predicted
        tmpVector = [B_ur_dim12(index,:),B(index,B_n),3];
        M_classify_dim12 = vertcat(M_classify_dim12,tmpVector);
    elseif (maxValue == A4_l2_dim12)   % train 4 predicted
        tmpVector = [B_ur_dim12(index,:),B(index,B_n),4];
        M_classify_dim12 = vertcat(M_classify_dim12,tmpVector);
    elseif (maxValue == A5_l2_dim12)   % train 5 predicted
        tmpVector = [B_ur_dim12(index,:),B(index,B_n),5];
        M_classify_dim12 = vertcat(M_classify_dim12,tmpVector);
    elseif (maxValue == A6_l2_dim12)   % train 6 predicted
        tmpVector = [B_ur_dim12(index,:),B(index,B_n),6];
        M_classify_dim12 = vertcat(M_classify_dim12,tmpVector);
    elseif (maxValue == A7_l2_dim12)   % train 7 predicted
        tmpVector = [B_ur_dim12(index,:),B(index,B_n),7];
        M_classify_dim12 = vertcat(M_classify_dim12,tmpVector);
    elseif (maxValue == A8_l2_dim12)   % train 8 predicted
        tmpVector = [B_ur_dim12(index,:),B(index,B_n),8];
        M_classify_dim12 = vertcat(M_classify_dim12,tmpVector);
    else                              % train 9 predicted
        tmpVector = [B_ur_dim12(index,:),B(index,B_n),9];
        M_classify_dim12 = vertcat(M_classify_dim12,tmpVector);
    end % end-if
end % end-for_each

M_classify_dim12_n = size(M_classify_dim12,2);
M_classify_dim12_m = size(M_classify_dim12,1);

% Konfusionsmatrix
knownClass_dim12 = M_classify_dim12(:, M_classify_dim12_n -1);
predictedClass_dim12 = M_classify_dim12(:, M_classify_dim12_n);
confusionmatrix_dim12 = confusionmat(knownClass, predictedClass)

%  confusionmatrix_dim12 =
%
%  341     0     0     0     0     0     0     0    22     0
%    0   350    12     0     1     0     0     0     1     0
%    0     8   355     0     0     0     0     1     0     0
...
% Klassifikationsguete
corret_predicted_dim12 = 0;
for index = 1:M_classify_dim12_m
    if M_classify_dim12(index, M_classify_dim12_n -1) == M_classify_dim12(index, M_classify_dim12_n)
        corret_predicted_dim12 = corret_predicted_dim12 + 1;
    end
end
classification_quality_dim12 = corret_predicted_dim12 / M_classify_dim12_m

%  classification_quality_dim12 = 0.9540